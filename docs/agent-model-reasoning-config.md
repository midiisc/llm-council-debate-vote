...
|---|---|---|---|---|
| Stage 1 — independent draft | opus-4.8, gpt-5.5 | **high** | substantive generation task — **but see status column: this is a judgment call, not a data-backed conclusion** | ✅ **Implemented and live 2026-08-14** via a new project-owned function (same raw-HTTP-plus-top-level-`reasoning_effort` pattern as `live_adapters.py`'s Contract 1-3 mechanism, not the package's unsafe nested-`reasoning_params` path — see `docs/specs/reasoning-effort-wiring-contract.md` Contract 4). Landed via blind-TDV (177/178 scoped mutants killed, 1 documented equivalent, 824/824 full suite green). **Two rollout-gating checks run**: (1) CSS comparison — moved 0.721→0.572, initially misread as a quality regression, corrected same day (CSS measures ranking agreement, not correctness — see `docs/pipeline-architecture-spec.md` §9). (2) Direct content-quality comparison (blinded self-judging against the Stage 2 rubric, `medium` vs `high`, both promoted seats) — **came back split 1-1**, no real signal either direction, modest/subjective differences, single trial. Cost is a wash ($0.362 vs $0.360). **Final: kept at `high` by explicit user decision made with full knowledge the data doesn't clearly support it** — not proof `high` is better. See `docs/upstream-deltas.md`'s final 2026-08-14 entry for the full arc. |
| Stage 1 — independent draft | gemini-3.6-flash, glm-5.2 | **medium** | Flash is a deliberate cost-tier pick; GLM hasn't graduated its 20-session bar — neither should be silently promoted to frontier-effort | ✅ implemented 2026-08-14 (Contract 4), unchanged by the dry-run (these two seats were always targeted at medium) |
